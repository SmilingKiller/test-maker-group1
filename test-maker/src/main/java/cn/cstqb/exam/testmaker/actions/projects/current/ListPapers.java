...
public class ListPapers extends BaseCurrentProjectAction {
    ...
    protected String doExecuteImpl() {
        logger.debug("ListPapers.doExecuteImpl: Loading papers for project: #0", sessionProject.getName() );
        papers = paperService.findAll(sessionProject, pageSize, pageNumber);
        logger.info(papers.get(0).getId()+"");
        logger.info(papers.get(0).getPaper_name());
        return null;
    }
